...
def txtinv_list(model):
    if (("XL" in model.upper()) or ("LIGHTNING" in model.upper()) or ("ETRI-VILAB/KOALA-" in model.upper()) or ("PLAYGROUNDAI/PLAYGROUND-V2-" in model.upper()) or ("SSD-1B" in model.upper()) or ("SEGMIND-VEGA" in model.upper()) or (model == "dataautogpt3/OpenDalleV1.1") or (model == "dataautogpt3/ProteusV0.4") or (model == "playgroundai/playground-v2-1024px-aesthetic")):
